...
function Levelup(win) {
	...
	back.addEventListener('click', function(e) {
		if(val != Ti.App.Properties.getInt('levelupMinimumPoint')){
			Ti.API.info(val);
			alert(L('Levelup_Minimum_Point_change') + '\n' + Ti.App.Properties.getInt('levelupMinimumPoint') + ' ' + L('Point') + ' > ' + val + ' ' + L('Point'));
			Ti.App.Properties.setInt('levelupMinimumPoint', val);
		}
		setTimeout(function(){
			win.remove(self);
		}, 100);
	...
	var levelupArray = [0, 3, 5, 10, 11, 12, 13, 14, 15];
	
	var val = Ti.App.Properties.getInt('levelupMinimumPoint');
    
    for(i=0; i<levelupArray.length; i++){
    		var row = Ti.UI.createTableViewRow({
		    height:'auto',
		...
		if(levelupArray[i] == Ti.App.Properties.getInt('levelupMinimumPoint')){
			leftImage.image = '/assets/images/Checkmark.png';
			row.backgroundColor = 'green';
		}
		/*
		leftImage.addEventListener('click', function(e) {
			if(e.source.val != Ti.App.Properties.getInt('levelupMinimumPoint')){
				Ti.API.info(e.source.val);
				alert(L('Levelup_Minimum_Point_change') + '\n' + Ti.App.Properties.getInt('levelupMinimumPoint') + ' ' + L('Point') + ' > ' + e.source.val + ' ' + L('Point'));
				Ti.App.Properties.setInt('levelupMinimumPoint', e.source.val);
				setTimeout(function(){
