function Products(win, title, bgColor, e) {
	...
	//UI
	var top = 0;
	var back, buttonWidth, label, product, Product, row, rows, self, 
		tableview, view, productImage;
	...
	*/
	...
	productObjects.forEach(function(product) {
		...
		productImage = Ti.UI.createImageView({
			image:'/assets/images/' + osname + '/' + product.SKU + '.png',
			width:scale(100),
			//top:20,
			left:scale(10),
		});
		if(osname=='ipad'){
			productImage.height = scale(75);
		}else{
			productImage.height = scale(56);
		}
