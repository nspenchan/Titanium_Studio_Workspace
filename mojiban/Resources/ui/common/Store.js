function Store(win, title, bgColor, arrayPurchase, textField, cursorImage, questionImage, adView) {
	//test
	...
	var ampm, back, buttonWidth, check, ContentName, errorMsg, 
		index, label, leftImage, line, message, productObjects, 
		Products, products, Purchases, purchases, row, rowFinal, self, 
		SKU, tableview, tf1, top, view;
		...
		if (e.errorCode) {
			...
			switch (e.purchase.state) {
				case InAppProducts.PURCHASE_STATE_PURCHASED:
					...
					if(adView.state==1){
						win.remove(adView);
						adView.state = 2;//iphoneのadIntervalを止める
					}
					break;
	...
	*/
	...
	var rows = [],
		arrayTitle = ['Welcome_to_kokucy_store_', 'Sales_Page', 'store_message', 'List_Products', 'Restore_Lost_Purchases', 'restore_message', 'List_Purchases'],
		arrayColor = ['#fff', '#ff7f7f', '#fff', '#ff7fbf', '#ff7fff', '#fff', '#bf7fff', '#7fbfff', '#7fffbf', '#7fff7f', '#bfff7f', '#ffff7f', '#ffbf7f'];
	...
	tf1.addEventListener('return',function(e){
		if(e.value=='999'){
			...
			for(i=0; i<productIDs.length; i++){
				...
				Ti.API.info(productIDs[i] + ' / ' + Titanium.App.Properties.getBool('Purchased-'+ productIDs[i]));
				if(osname=='ipad'){
					textField.height = 180;
					cursorImage.bottom = 50;
					questionImage.width = 140;
			...
			for(i=0; i<productIDs.length; i++){
				...
				//storeImage.image = '/assets/images/Shopping_Cart_Add.png';
			}
			//label_pro.opacity = 0;
