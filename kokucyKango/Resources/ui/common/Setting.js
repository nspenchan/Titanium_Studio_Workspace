function Setting(win, title, bgColor, dbName, arrayPurchase, bgColor1, arrayColor, arrayTest, arrayPurchase, db, db1, label01) {
	var top = 0;
	...
	var about, alertDialog, back, body, buttonWidth, children, create, data, Delete, 
		emailDialog, file, Firstview, firstview, History2, history2, id, index, 
		insert, label, lang, leftImage, mail, manual, Range, range, restart, 
		row, rows, select, self, state, Store, store,switch1, tableview, Timer, 
		timer, view, Webview, Zoom, zoom;
	var arrayTitle = ['Timer', 'Range', 'Zoom', 'Speed', 'About', 'Manual', 'Store'],
		arrayBool = ['noVoice', 'noSound', 'noVibrate', 'noRecord', 'random'],
		arrayLeft = ['Clock.png', 'Refresh02.png', 'Zoom.png', 'Announcement.png', 'Info.png', 'Help.png', 'shopping_basket.png'];
	...
	select = 'SELECT subject FROM ' + dbName + ' WHERE bang = 1 AND name = "' + Titanium.App.Properties.getString('test') + '"';
	...
	for(i=0; i<arrayBool.length; i++){
		...
		switch1.addEventListener('change', function(e){
		    ...
		    if(e.value==true){
		    		...
		    		if(osname=='android' && e.source.getbool=='noVoice'){
		    			alert(L('require_tts'));
		    		}
		    }
		});
	...
	Delete.addEventListener('click', function(e){
		...
		alertDialog.addEventListener('click',function(event){
		    ...
		    if(event.index == 0){
				...
				create = 'ID INTEGER PRIMARY KEY, DATE TEXT, CORRECT INTEGER, STARS INTGER, MEMO TEXT';
				...
				for(i=1; i<=3000; i++){
					insert = 'INSERT INTO record (ID) values (' + i + ')';
					db1.execute(insert);
				}
				db1.close();
				setTimeout(function(){
	...
	mail.addEventListener('click', function(e){
		...
		// 題名の初期値をセットします
		emailDialog.setSubject(L('Countact_us') + ': kokucy' + data.field(0) + '国家試験');
	...
	restart.addEventListener('click', function(e) {
		...
		setTimeout(function(){
			...
			//construct UI
			firstView = new FirstView(dbName, bgColor1, arrayTest, arrayPurchase);
			win.add(firstView);
		}, 1000);
	});
	...
	tableview.addEventListener('click', function(e){
	    ...
	    switch(id){
				...
				//construct UI
				range = new Range(win, arrayTitle[index], arrayColor[index], label01);
				win.add(range);
	    		break;
	    		case 'Zoom':
				Zoom = require('ui/common/Zoom');
				//construct UI
				zoom = new Zoom(win, arrayTitle[index], arrayColor[index], dbName, bgColor1, arrayTest, arrayPurchase, arrayColor);
				win.add(zoom);
	    		break;
	    		case 'Speed':
				Speed = require('ui/common/Speed');
				//construct UI
				speed1 = new Speed(win, arrayTitle[index], arrayColor[index]);
				win.add(speed1);
	    		break;
	    		case 'About':
				...
				win.add(store);
				/*
				window1 = Ti.UI.createWindow({ url:"/lib/inappproducts.js"});
				window1.open();
				*/
				/*
				(function() {
					Window = require('lib/inappproducts.js');
					new Window().open();
				})();
				*/
	    		break;
