function Result(win, title, bgColor, dbName, arrayPurchase, arrayRecord, db, db1, label01, continueMode) {
	var top = 0,
		arrayId = [],
		correctNum = 0,
		range = Titanium.App.Properties.getInt('range');
	var alertDialog, ampm, back, buttonWidth, children, label, labelTop, leftImage, 
		Question, question, questionTimes, retry, row, rows, section, self, 
		tableview, test, view;
	...
	retry.addEventListener('click', function(e){
		...
		if(arrayId.length==0){
			...
			//construct UI
			question = new Question(win, 'Result_', bgColor, dbName, arrayPurchase, [], arrayId, -1, '', db, db1);
			win.add(question);
	...
	if(Titanium.App.Properties.getInt('max')>Titanium.App.Properties.getInt('from')+10 && continueMode==true){
	...
	back.addEventListener('click', function(e){
		...
		if(continueMode==true){
			alertDialog = Titanium.UI.createAlertDialog({
			    title: L('Continue'),
			    message: L(test.substring(0, 4)) + test.substring(4, 7) + '回' + ampm + '\n' + L('continue_message'),
			...
			alertDialog.addEventListener('click',function(event){
			    ...
			    if(event.index == 1){
			        // cancel時の処理
			        alert(L(test.substring(0, 4)) + test.substring(4, 7) + '回' + ampm + '\n' + L('stop_message'));
					db.close();
					db1.close();
					setTimeout(function(){
						//ホーム画面以外をすべて消す
			    ...
			    if(event.index == 0){
			        // "OK"時の処理
			        bang = Titanium.App.Properties.getInt('from') + range -1;
		    			if((Titanium.App.Properties.getInt('max')-bang)<range){
		    				questionTimes = (Titanium.App.Properties.getInt('max')-Titanium.App.Properties.getInt('min'))%range + 1;
		    			}else{
		    				questionTimes = range;
		    			}
					Titanium.App.Properties.setInt('from', Titanium.App.Properties.getInt('from') + range);
					Question = require('ui/common/Question');
					//construct UI
					question = new Question(win, 'Question', '#ff7f7f', dbName, arrayPurchase, [], [], -1, bang, db, db1, questionTimes, label01);
					win.add(question);
					
					setTimeout(function(){
				        label01.text = L('ExamFrom') + Titanium.App.Properties.getInt('from') + L('ExamFrom2') 
							+ Titanium.App.Properties.getInt('min') + '-' + Titanium.App.Properties.getInt('max') + ' )';
						//win.remove(self);
						//ホーム画面と出題画面以外をすべて消す
						children = win.getChildren();
	...
	for(i=0; i<arrayRecord.length; i++){
		...
		row = Ti.UI.createTableViewRow({
		    height:'auto',
		    backgroundColor:arrayRecord[i][2],
		    id:arrayRecord[i][0],
		    check:0,
		});
