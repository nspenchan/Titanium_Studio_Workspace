function Zoom(win, title, bgColor, dbName, bgColor1, arrayTest, arrayPurchase, arrayColor) {
	var top = 0;
	var selfZoom = Titanium.App.Properties.getDouble('selfZoom');
	var arrayZoom, back, buttonWidth, children, FirstView, firstView, label, 
	...
	back.addEventListener('click', function(e){
		...
		if(Titanium.App.Properties.getDouble('selfZoom')!=selfZoom){
			...
			setTimeout(function(){
				...
				//construct UI
				firstView = new FirstView(dbName, bgColor1, arrayColor, arrayTest, arrayPurchase);
				win.add(firstView);
			}, 1000);
		}else{
			setTimeout(function(){
	...
	if(osname !='ipad'){
		arrayZoom = [0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.5];
	}else{
		arrayZoom = [0.5, 0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.5, 2];
	}
